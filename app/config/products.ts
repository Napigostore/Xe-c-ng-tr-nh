import { salesConfig } from "./salesConfig";

export const products = [
  {
    name: "Combo 3 xe công trình: Xúc XUEZHISHAN + Ben XUEZHISHAN + Ủi YIGONG",
    model: "Bộ 3 xe điều khiển từ xa: máy xúc XUEZHISHAN, xe ben XUEZHISHAN và xe ủi YIGONG",
    price: "1.199.000đ",
    schemaPrice: "1199000",
    rating: "4.9",
    sold: "Combo tiết kiệm",
    reviews: "Ưu đãi 20% so với mua lẻ",
    image: "/assets/products/combo-xe-cong-trinh.png",
    badge: "Tiết kiệm 401.000đ",
    highlights: ["Giá gốc 1.600.000đ", "Giá KM 1.199.000đ", "Bộ công trình đầy đủ"],
    href: `${salesConfig.shopUrl}?keyword=combo%203%20xe%20cong%20trinh`,
  },
  {
    name: "Combo 3 xe công trình YIGONG: Xúc + Ben + Ủi",
    model: "Bộ 3 xe điều khiển từ xa thương hiệu YIGONG: máy xúc, xe ben và xe ủi",
    price: "1.299.000đ",
    schemaPrice: "1299000",
    rating: "4.9",
    sold: "Combo mới",
    reviews: "Giá gốc 1.700.000đ",
    image: "/assets/products/combo-3-xe-yigong.png",
    badge: "Tiết kiệm 401.000đ",
    highlights: ["Giá gốc 1.700.000đ", "Giá KM 1.299.000đ", "Full combo YIGONG"],
    href: `${salesConfig.shopUrl}?keyword=combo%203%20xe%20yigong`,
  },
  {
    name: "Máy xúc điều khiển từ xa YIGONG",
    model: "11 kênh, gầu hợp kim, tỉ lệ 1:20",
    price: "599.000đ",
    schemaPrice: "459000",
    rating: "4.9",
    sold: "5664 đã bán",
    reviews: "884 đánh giá",
    image: "/assets/products/xuc-yigong.webp",
    badge: "Bán chạy",
    highlights: ["2.4GHz", "Gầu hợp kim", "Mô phỏng chân thực"],
    href: `${salesConfig.shopUrl}?keyword=m%C3%A1y%20x%C3%BAc%20yigong`,
  },
  {
    name: "Xe tải điều khiển từ xa XUEZHISHAN",
    model: "Thùng hợp kim nâng 65 độ, tỉ lệ 1:20",
    price: "569.000đ",
    schemaPrice: "429000",
    rating: "4.9",
    sold: "2156 đã bán",
    reviews: "652 đánh giá",
    image: "/assets/products/ben-xuezhishan.webp",
    badge: "Thùng nâng 65 độ",
    highlights: ["Điều khiển từ xa", "Chi tiết sắc nét", "Quà tặng hấp dẫn"],
    href: `${salesConfig.shopUrl}?keyword=xuezhishan%20xe%20t%E1%BA%A3i`,
  },
  {
    name: "Xe ủi điều khiển từ xa YIGONG",
    model: "9 kênh, gầu hợp kim, tỉ lệ 1:20",
    price: "579.000đ",
    schemaPrice: "459000",
    rating: "4.9",
    sold: "2142 đã bán",
    reviews: "651 đánh giá",
    image: "/assets/products/ui-yigong.webp",
    badge: "Gầu hợp kim",
    highlights: ["9 chức năng", "Hỏa tốc", "Bảo hành uy tín"],
    href: `${salesConfig.shopUrl}?keyword=xe%20%E1%BB%A7i%20yigong`,
  },
  {
    name: "Xe tải điều khiển từ xa YIGONG",
    model: "9 kênh, thùng hợp kim nâng 45 độ, tỉ lệ 1:20",
    price: "599.000đ",
    schemaPrice: "439000",
    rating: "4.8",
    sold: "2306 đã bán",
    reviews: "724 đánh giá",
    image: "/assets/products/ben-yigong.webp",
    badge: "Giá tốt",
    highlights: ["9 chức năng", "Thùng nâng", "Giao hỏa tốc"],
    href: `${salesConfig.shopUrl}?keyword=yigong%20xe%20t%E1%BA%A3i`,
  },
  {
    name: "Máy xúc điều khiển từ xa XUEZHISHAN",
    model: "11 kênh, gầu hợp kim, tỉ lệ 1:20",
    price: "589.000đ",
    schemaPrice: "439000",
    rating: "5.0",
    sold: "6170 đã bán",
    reviews: "1073 đánh giá",
    image: "/assets/products/xuc-xuezhishan.webp",
    badge: "Cao cấp",
    highlights: ["11 kênh", "Phụ kiện đi kèm", "Quà sinh nhật cho bé"],
    href: `${salesConfig.shopUrl}?keyword=m%C3%A1y%20x%C3%BAc%20xuezhishan`,
  },
];
